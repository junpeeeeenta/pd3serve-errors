(in-package :cl-user)
(defpackage pd3serve-errors
  (:nicknames :lack.middleware.clack.errors)
  (:use :cl)
  (:import-from :trivial-backtrace
                :print-backtrace)
  (:export :*clack-error-middleware*
           :*lack-middleware-pd3serve-errors*))
(in-package :pd3serve-errors)

(defparameter *dev-css-path*
  (merge-pathnames
   #p"static/style-dev.css"
   (asdf:component-pathname (asdf:find-system :pd3serve-errors))))
...
(defparameter +dev-template+
  (djula:compile-template*
   (asdf:system-relative-pathname :pd3serve-errors #p"templates/dev-page.html")))

(defparameter +prod-template+
  (djula:compile-template*
   (asdf:system-relative-pathname :pd3serve-errors #p"templates/prod-page.html")))
...
(setf (symbol-value '*lack-middleware-pd3serve-errors*) *clack-error-middleware*)
